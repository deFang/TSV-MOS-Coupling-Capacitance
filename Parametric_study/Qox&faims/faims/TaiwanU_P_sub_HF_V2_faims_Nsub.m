...
Qox=[-7.08516807909393e-13,-5.89740852721369e-13,1.24821772062079e-12,1.31082545986398e-12,3.37136961835086e-12];%Qox is the charges in silicon surface
Qox=-Qox
m=[0.0425,0.0475,0.0425,0.0425,0.0425]
phiB=0.026*log(Na/ni)-m;
para_radius_vol=zeros(1000,5);
para_radius_cap=zeros(1000,5);
...
%% HF for TSV1
for i=2:range-1
    if (electricfield1(i,2)-(2*phiB(par)))*(electricfield1(i+1,2)-(2*phiB(par)))<=0
        thresh=i
        break
    end
end
...
%% HF for TSV2
for i=2:range-1
    if (electricfield2(i-1,2)>0)&((electricfield2(i,2)-(2*phiB(par)))*(electricfield2(i+1,2)-(2*phiB(par)))<=0)
        thresh=i
        break
    end
end
