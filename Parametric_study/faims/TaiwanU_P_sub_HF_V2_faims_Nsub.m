...
%%% Parametric study-oxide charges
faim=[4.05,4.1,4.65,4.67,5.17];%n+ poly, Al, Cu,W,p+ poly
% for n-sub
faims=faim-(4.05+0.56-0.026*log(Na/ni))
para_radius_vol=zeros(1000,6);
para_radius_cap=zeros(1000,6);
...
E=fais(i)/R+sqrt((fais(i)/R)^2+2*F);
...
%charge amount
electricfield1(i,3)=2*pi*R*h*dielectSi*electricfield1(i,1);
%voltage on liner
electricfield1(i,4)=-electricfield1(i,3)/cox;
% voltage of TSV
...
%Ctsv
electricfield1(i,7)=1/((1/electricfield1(i,6))+(1/cox));
end
figure(1)
...
for par=1:5
for i=2:range-1
    if (electricfield1(i,5)-faims(par))*(electricfield1(i+1,5)-faims(par))<0
        Qox(par)=-2*electricfield1(i,3);
    end
end
end
...
for i=2:range
    %charge on TSV2
    electricfield2(i,3)=Qox(par)-electricfield1(i,3);
    %electric field
    E=electricfield2(i,3)/(2*pi*R*h*dielectSi)
    electricfield2(i,1)=E;
    % voltage of liner
    electricfield2(i,4)=-electricfield2(i,3)/cox;
      for j=2:range-1
...
%Ctsv
electricfield2(i,7)=1/((1/electricfield2(i,6))+(1/cox));
end
figure(2)
...
bench_cap(par)=1/((1/electricfield2(thresh,6))+(1/cox)+(1/cox))
figure(2)
plot(electricfield2(:,5),electricfield2(:,7))
%% CV curve for TSV pair
electricfield1(:,8)=smooth(electricfield1(:,7),10);
...
for m=1:mat(1)*2
para_radius_vol(m,par)=tsv_hf(m,1);
para_radius_cap(m,par)=tsv_hf(m,4)/bench_cap(par);
end
clear tsv_hf
end
plot(para_radius_vol(:,1),para_radius_cap(:,1),para_radius_vol(:,2),para_radius_cap(:,2),para_radius_vol(:,3),para_radius_cap(:,3),para_radius_vol(:,4),para_radius_cap(:,4),para_radius_vol(:,5),para_radius_cap(:,5),para_radius_vol(:,6),para_radius_cap(:,6))
